(function (global, factory) {
        ...
        (commonjsGlobal,function(){
            ...
            var l = function(t,i){
                ...
                if(Symbol.iterator in Object(t))return function(t,i){            
                    ...
                    try{
                        for(var r, h = t[Symbol.iterator]();
                        !(s = (r = h.next()).done) && (e.push(r.value),!i || e.length !== i); s = !0);
                    } catch(t){
                        o =! 0,n = t;
                    } finally{
                        try{
            ...
            var i = t && /wechatdevtools/.test(t);
            var s = t && 0 < t.indexOf("android");
            ...
            function m(t){
                for(var i=0,e=0;t;)i -= t.offsetLeft,e -= t.offsetTop,t=t.offsetParent;return {
                    left:i,
                    top:e
                }
            }
            ...
            var d = h("transform");
            var f = h("transition");
            var v = e&&h("perspective")in n;
            var g = e&&("ontouchstart"in window||i);
            var y =! 1 !== d;
            var w = e&&f in n;
            var S = {
                transform:d,
            ...
            function X(t,i){
                for(var e in i)
                    if(i[e].test(t[e]))return !0;return !1
            }
            ...
            var k,P,L,O,W,H = e ? window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || function(t){
                return window.setTimeout(t,(
                    t.interval||100/60)/2)
                }:D,z = e?window.cancelAnimationFrame||window.webkitCancelAnimationFrame||window.mozCancelAnimationFrame||window.oCancelAnimationFrame||function(t){
                ...
                k.prototype.refresh = function(){
                    ...
                    this.minScrollX =- this.relativeX): 1 < this.scale&&(this.maxScrollX=this.maxScrollX / 2 - this.relativeX,this.minScrollX = this.maxScrollX),
                        ...
                        if(!(r-this.endTime > this.options.momentumLimitTime && n < this.options.momentumLimitDistance && o < this.options.momentumLimitDistance)){
                            if(this.directionLocked || this.options.freeScroll || (o > n + this.options.directionLockThreshold ? this.directionLocked = "h":n>=o+this.options.directionLockThreshold?this.directionLocked="v":this.directionLocked="n"),"h"===this.directionLocked){
                                if("vertical" === this.options.eventPassthrough)t.preventDefault();
                                    else if("horizontal" === this.options.eventPassthrough)return void(this.initiated=!1); s = 0;
                            } else if("v" === this.directionLocked){
                                if("horizontal" === this.options.eventPassthrough)
                                    t.preventDefault();
                                else if("vertical" === this.options.eventPassthrough)return void(this.initiated=!1); e = 0;
                            ...
                            this.moved || (this.moved =! 0,this.trigger("scrollStart")),this._translate(h,a),r-this.startTime > this.options.momentumLimitTime && (this.startTime = r,this.startX = this.x,this.startY = this.y,1 === this.options.probeType && this.trigger("scroll",{
                                x:this.x,
                                y:this.y
                            })),
                            ...
                            var d = document.documentElement.scrollLeft || window.pageXOffset || document.body.scrollLeft,f = document.documentElement.scrollTop || window.pageYOffset||document.body.scrollTop,v = this.pointX-d,g = this.pointY-f;(v>document.documentElement.clientWidth-this.options.momentumLimitDistance||v < this.options.momentumLimitDistance || g < this.options.momentumLimitDistance || g > document.documentElement.clientHeight-this.options.momentumLimitDistance)&&this._end(t);}}},P.prototype._end=function(t){
                                if(this.enabled && !this.destroyed && x[t.type] === this.initiated){
                                    ...
                                    if(this.directionX = 0 < s?-1:s < 0 ? 1:0,
                                        this.directionY = 0 < o ? -1: o < 0 ? 1:0,
                                        !this.options.pullDownRefresh || !this._checkPullDown())if(this._checkClick(t))this.trigger("scrollCancel");
                                        ...
                                        if(i!==this.x||e!==this.y)
                                            return (i > this.minScrollX || i < this.maxScrollX || e > this.minScrollY || e < this.maxScrollY) && (y = M.swipeBounce),
                ...
                P.prototype._transitionEnd = function(t){
                    t.target === this.scroller && this.isInTransition && (
                            this._transitionTime(),(!this.pulling || 1 === this.movingDirectionY) && !this.resetPosition(
                                this.options.bounceTime,M.bounce) && (
                                    this.isInTransition=!1,3 !== this.options.probeType&&this.trigger(
                                        "scrollEnd",{
                                            x:this.x,y:this.y
                                        }
                                    )
                ...
                P.prototype._animate=function(r,h,a,l){
                    ...
                    z(this.animateTimer),function t(){
                        var i=T();
                        if(v <= i)
                            return c.isAnimating=!1,c._translate(r,h,d),c.trigger("scroll",{x:c.x,y:c.y}),
                                void(c.pulling||c.resetPosition(c.options.bounceTime)||c.trigger("scrollEnd",{x:c.x,y:c.y}));
                        var e=l(i=(i-f)/a),s=(r-p)*e+p,o=(h-u)*e+u,n=(d-m)*e+m;c._translate(s,o,n),c.isAnimating&&(c.animateTimer=H(t)),3===c.options.probeType&&c.trigger("scroll",{x:c.x,y:c.y});
                    }();
                },
                
                P.prototype.scrollBy = function(t,i){
                    var e = 2 < arguments.length && void 0 !== arguments[2]?arguments[2]:0,s = 3 < arguments.length && void 0 !== arguments[3] ? arguments[3]:M.bounce;
                ...
                P.prototype.scrollTo=function(t,i){
                    ...
                    !e || this.options.useTransition ? (this._transitionTimingFunction(s.style),this._transitionTime(e),this._translate(t,i),e && 3 === this.options.probeType&&this._startProbe(),e||(this.trigger("scroll",{x:t,y:i}),
                    
                    this._reflow = document.body.offsetHeight,this.resetPosition(this.options.bounceTime,M.bounce)||this.trigger("scrollEnd",{x:t,y:i})),
                ...
                P.prototype.resetPosition=function(){
                    var t = 0 < arguments.length && void 0 !== arguments[0] ? arguments[0]:0,
                            ...
                            e = this.x,
                            s = Math.round(e);
                    !this.hasHorizontalScroll||s>this.minScrollX?e=this.minScrollX:s<this.maxScrollX&&(e=this.maxScrollX);var o=this.y,n=Math.round(o);
                ...
                P.prototype.getComputedPosition=function(){
                    var t = window.getComputedStyle(this.scroller,null),i=void 0,e=void 0;return e=this.options.useTransform?(i=+((t=t[S.transform].split(")")[0].split(", "))[12]||t[4]),+(t[13]||t[5])):(i=+t.left.replace(/[^-\d.]/g,""),+t.top.replace(/[^-\d.]/g,"")),{x:i,y:e}
                },
                ...
                P.prototype.stop=function(){
                    if(this.options.useTransition&&this.isInTransition){
                        ...
                        this.options.wheel ? this.target = this.items[Math.round(-t.y/this.itemHeight)]:this.trigger("scrollEnd",{x:this.x,y:this.y}),
                        this.stopFromTransition=!0;
                    }
                    
                    else!this.options.useTransition&&this.isAnimating&&(this.isAnimating=!1,z(this.animateTimer),this.trigger("scrollEnd",{x:this.x,y:this.y}),
                    this.stopFromTransition=!0);
                },
                ...
                W.prototype._onMouseWheel=function(t){
                    var i=this;if(this.enabled){
                        ...
                        s = e.speed,
                        o = void 0===s?20:s,n=e.invert,r=void 0!==n&&n,h=e.easeTime,a=void 0===h?300:h;
                        ...
                        if(this.options.snap)
                            return u = this.currentPage.pageX,
                            m = this.currentPage.pageY,0 < l?u--:l<0&&u++,0 < c ? m--:c < 0 && m++,
                            ...
                            u > this.minScrollX ? u = this.minScrollX:u < this.maxScrollX&&(u = this.maxScrollX),m > this.minScrollY ? m = this.minScrollY:m < this.maxScrollY&&(m=this.maxScrollY);
                            var d = this.y === m;
    ...
    var version = "3.1.3";
		...
		function Rolldate(config = {}) {
			...
			var date = config.value ? new Date(config.value.replace(/-/g, '/')) : new Date();
			
			if (isNaN(date.getTime())) {
				console.error('Invalid Date: ' + config.value);
				date = new Date();
			}
		...
		Rolldate.prototype = {
			...
			extend: function extend(config) {
				var opts = this.baseData().opts;
				for (var key in opts) {
					this.config[key] = config[key] || opts[key];
				}
			},
			...
			createUI: function createUI() {
				...
				var domId = _this.baseData().domId;
				var FormatArr = config.format.split(/-|\/|\s|:/g);
				var len = FormatArr.length;
				var ul = '';
				var date = _this.bindDate || new Date();
				var itemClass = '';
				...
				var txt = '<div class="rolldate-wrapper"><div class="mask-top"></div>' + ul + '<div class="mask-bottom"></div></div>';
				txt += '<div id="rolldate-buttons"><button type="button" id="rolldate-confirm">Zoek</button></div>';

				document.getElementById('date-search').innerHTML = txt;

				_this.scroll = {};

				FormatArr.forEach(function (f) {
					var $id = domId[f];
					_this.scroll[f] = new bscroll_min('#' + $id, {
						wheel: {
							selectedIndex: 0
						}
					});
					...
					var index = active ? parseInt(active.getAttribute('data-index'), 10) : 0;

					_this.scroll[f].wheelTo(index);
					_this.scroll[f].on('scrollEnd', function () {
						if (config.moveEnd) {
							config.moveEnd.call(_this, _this.scroll[f]);
						}
					});
				});
			},
			...
			show: function show() {
				var _this = this;
				if (_this.config.init && _this.config.init.call(_this) === false) {
					return;
				}
				_this.createUI();
				_this.event();
			...
			event: function () {
				var _this = this;
				var cancel = document.getElementById('rolldate-cancel');
				var confirm = document.getElementById('rolldate-confirm');

				this.tap(cancel, function () {
					window.location = "/pages/song_search.html"; // Or any action for cancel
				});

				this.tap(confirm, function () {
					var config = _this.config;
					var date = config.format; // Start with the format
					var newDate = new Date();

					// Retrieve values from the scroll instances
					for (var f in _this.scroll) {
						var selectedValue = _this.getSelected(_this.scroll[f]); // Get the selected value
						date = date.replace(f, selectedValue); // Replace the placeholder with the selected value
						if (f === 'YYYY') {
							newDate.setFullYear(selectedValue); // Set the selected year
						} else if (f === 'MM') {
							newDate.setMonth(selectedValue - 1); // Set the selected month (0-based)
						}
					}

					if (config.confirm) {
						var flag = config.confirm.call(_this, date); // Call the confirm callback
						if (flag === false) {
							return false;
						} else if (flag) {
							date = flag;
					...
					// Create a query string with the selected date values
					let newString = date.replace(/-/g, ''); // Format date for the URL
					window.location = '/pages/song_list.html?date=' + newString; // Redirect with the selected date
				});
			},
			...
			// event: function event() {
				...
				// // _this.tap(cancel, function () {
					...
					// // if (_this.config.cancel) {
						// // _this.config.cancel.call(_this);
					// // }
				// // });

				// _this.tap(confirm, function () {
					// var config = _this.config;
					// var date = config.format;
					// var newDate = new Date();
					...
					// let newSearchDate = date.replace('-', ''); 

					// if (config.confirm) {
						// config.confirm.call(_this, newSearchDate);
					// }
				// });
			// },
			...
			getSelected: function (scroll) {
				...
				return scroll.wrapper.querySelectorAll('li')[selectedIndex].innerText;
			}
		};
